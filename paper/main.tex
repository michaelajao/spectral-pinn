...
the boundary conditions~\cite{son2022alpinn}. A third account, the one this
...
our own reference against. Section~\ref{sec:refcheck} does exactly that, and it
is the part of this paper we did not plan.

This paper takes the three limitations in order and adds the excluded regime.
Our results are largely negative, and we think the negative results are the
useful part.
...
AL-PINNs~\cite{son2022alpinn} instead treat the initial and boundary data as
...
physically admissible weak solution~\cite{deryck2022wpinn}; other approaches
...
\subsection{The regimes compared}

Every regime below shares Eq.~\eqref{eq:factor} and keeps $V = V_0$ frozen;
they differ in what is trained and in what constrains $U$.

\begin{description}
  \item[Unconstrained.] $W$ trained directly, with no factorisation. The
    baseline.
  \item[Penalised spectral.] $U$ and $s$ trained, with $\wU \sum_j \DU_j^2$
    from Eq.~\eqref{eq:penalty} added to the loss. This is the published
    method, and $\wU$ is its one free parameter.
  \item[Exactly orthogonal.] $U$ confined to the Stiefel manifold by the
    matrix-exponential parametrisation of Lezcano-Casado and
    Mart\'inez-Rubio~\cite{lezcano2019orthogonal}, and $s$ trained. The
    constraint holds to machine precision at every step, so the penalty term
    and $\wU$ do not exist. This is the direct test of the first stated
    limitation: if the penalty is an approximation to orthogonality, enforcing
    orthogonality exactly should do at least as well and needs no tuning.
  \item[Spectrum only.] $U$ and $V$ both frozen at their orthogonal initial
    values, $s$ alone trained --- $r$ parameters per layer instead of $mn$, and
    exact spectral control by construction. Structurally this is the layer of
    SVD-PINNs~\cite{gao2022svdpinn}, where it transfers a trained network to a
    new equation; here it is trained from scratch. The construction is not ours
    and is not presented as new.
  \item[Bounded $\sigma_{\max}$.] $U$ rescaled on every forward pass so its
    largest singular value is 1, with the others left free in $[0,1]$. Strictly
    weaker than orthogonality, and no penalty. This regime is not part of the
    original design; it is introduced in Section~\ref{sec:bounded} because the
    measurements of Section~\ref{sec:mechanism} predict it should work, and
    reporting it is how that prediction gets tested.
\end{description}

The pivot of the paper is the pair formed by the second and third regimes. They
target the same constraint --- one softly with a tuned weight, one exactly and
for free --- so if the penalty does what the source paper says it does, they
should agree. They do not, and the rest of the paper is about why.
...
implemented it as a projection and could not train it under this paper's
protocol, for a reason worth recording. The projection is exact --- the
constrained networks have every singular value inside the box to five decimal
places --- but its Jacobian is ill-conditioned precisely where the layer
operates: differentiating through the singular value decomposition introduces
...
exactly-orthogonal regime from the same initialisation. We report this as an
open implementation problem rather than as evidence about the constraint class,
which it is not.
...
enough to make the box trainable is untested; that is the next thing to try.
